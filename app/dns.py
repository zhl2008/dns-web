...
con=MySQLdb.connect(host=mysql_host,user=mysql_user,passwd=mysql_passwd,db=mysql_db,port=mysql_port)
redis_con=redis.Redis(host=redis_ip,port=redis_port,password=redis_pass,db=0)

def get_result_from_redis(domain):
    return redis_con.get("domain:"+domain)
    
def get_modules(domain):
    cursor=con.cursor()
    domain_depth=len(domain.split('.'))
    for i in xrange(domain_depth):
        sql="select module_name from modules where domain='"+domain+"' order by add_time desc limit 0,1"
        cursor.execute(sql)
        module_name=cursor.fetchall()
	domain=domain[domain.find('.')+1:]
        if  module_name:
            print 'module find:'+module_name[0][0]
...
def check_valid(data):
    for i in range(len(data)):
        if  data[i]=='.' or  ( data[i]>='0' and  data[i]<='9'):
            pass
        else:
            return '2.2.2.2'
    return data

def write_to_mysql(domain,result):
    cursor=con.cursor()
    sql="insert into dns   (domain,server,ip) values ('"+domain+"','10.0.0.10','"+result+"');"
    cursor.execute(sql)
    con.commit() 
    
def write_to_redis(domain,result):
    redis_con.set('domain:'+domain,result)

def myproxy(domain,header_id):
    result=get_result_from_redis(domain)
    if result:
	print 'from redis:'+result
        result=check_valid(result)
    else:
        module_name=get_modules(domain)
        data=run_modules(module_name)
        result=check_valid(data)
        write_to_mysql(domain,result)
        write_to_redis(domain,result)
    reply = DNSRecord(DNSHeader(id=header_id,qr=1,aa=1,ra=1),q=DNSQuestion(domain),a=RR(domain,rdata=A(result)))
    return str(reply.pack())
        ...
        try:
            ...
            if QR[d.header.qr] == "QUERY":  
                ...
                # Proxy the request
                    ...
                    #print nameserver_tuple  
                    #print self.server.nameservers             
                    response = self.proxyrequest(data,*nameserver_tuple)
                    response = myproxy(qname,d.header.id)
        return response         
